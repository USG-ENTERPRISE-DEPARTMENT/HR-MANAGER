...
/**
 * Editor for the medical multi-stage approval flow. Each stage has a name and an approver — either a
 * role (anyone with it) or a specific user. Saved to PUT /medical/approval-flow. A submitted medical
 * request (staff, dependent, or hospital claim) then walks these stages in order before it can be approved.
 */
export function MedicalApprovalFlow({ onClose }: { onClose: () => void }) {
  ...
  return (
    <FormModal
      title="Medical Approval Flow"
      subtitle="A submitted medical request must be approved at each stage below, in order, before it can be finalized."
      onClose={onClose}
      onSave={save}
        ...
        <div className="space-y-3">
          {stages.length === 0 && (
            <div className="rounded-lg border border-dashed border-[var(--border)] px-4 py-6 text-center text-[13px] text-[var(--text-muted)]">
              ...
              No approval stages yet. Add the first stage — a request will need one sign-off per stage.
            </div>
          )}
              ...
              <div key={i} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <input
                    ...
                    placeholder={`Stage ${i + 1} name (e.g. Medical Officer Review)`}
                  />
