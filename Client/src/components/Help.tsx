    ...
    articles: [
      {
        ...
        summary: 'Understand how the organisation structure is built and where it is used across the system.',
        icon: BookOpen,
        content: [
          { type: 'text', body: 'The Company module (shown as "Organization Structure") holds your organisation\'s hierarchy. Everything lives in one list — companies, branches, departments, units and teams are all structures, distinguished by a Type field rather than by separate pages.' },
          {
            type: 'table',
            headers: ['Tab', 'What it shows'],
            rows: [
              ['Company Structure', 'The full list of structures, with search and a Type filter. Add, edit and delete happen here.'],
              ['Organogram',        'A visual tree of those structures, arranged by their parent links.'],
            ],
          },
          {
            type: 'table',
            ...
            rows: [
              ['Company',    'The top-level entity, usually with no parent'],
              ['Branch',     'A physical or geographic location'],
              ['Department', 'A functional grouping — referenced on employee profiles and in report filters'],
              ['Unit',       'A sub-grouping within a department or branch'],
              ['Team',       'The finest grouping'],
            ],
          },
          { type: 'tip', body: 'Job titles are not managed here. They are a code list, maintained under System settings.' },
        ],
      },
      {
        id: 'departments',
        title: 'Adding a Structure',
        summary: 'Create a company, branch, department, unit or team.',
        icon: FileText,
        content: [
          { type: 'text', body: 'All structures are created the same way — the Type field decides what it is. Departments created here become available on employee profiles and in leave rules.' },
          {
            type: 'steps', heading: 'Adding a structure',
            steps: [
              { label: 'Go to Company from the sidebar and stay on the Company Structure tab.' },
              { label: 'Click Add and give it a Title.' },
              { label: 'Choose the Type — Company, Branch, Department, Unit or Team.' },
              { label: 'Choose a Parent to place it in the hierarchy. Leave the parent empty for a top-level entry.' },
              { label: 'Optionally set a code, address, description and the head of the structure, then save.' },
            ],
          },
          { type: 'tip', body: 'The structure code can be generated for you — the auto-generate setting lives in Settings. Use the Type filter and the search box on the list to find an existing structure by title or code.' },
          { type: 'warning', body: 'A structure cannot be its own parent, and the parent you choose cannot sit underneath it. Either is refused with a circular-reference error.' },
        ],
      },
      {
        id: 'delete-structure',
        title: 'Deleting a Structure',
        summary: 'What is checked before a structure can be removed — and what is not.',
        icon: FileText,
        content: [
          { type: 'text', body: 'Deleting is blocked only when the structure has children. Move or delete everything underneath it first, then delete the structure itself.' },
          { type: 'warning', body: 'Employees assigned to a structure do NOT block deletion. Deleting a department that still has employees in it will succeed, and those employees are left pointing at a structure that no longer exists. Reassign them before you delete.' },
        ],
      },
      {
        id: 'organogram',
        title: 'Viewing the Organogram',
        summary: 'See your organisation structure as a visual tree.',
        icon: Building2,
        content: [
          { type: 'text', body: 'The Organogram draws the structures you have created, connected by their parent links. It is a chart of the organisation, not of individual employees — each node is a company, branch, department, unit or team, colour-coded by type.' },
          {
            type: 'steps', heading: 'Using the organogram',
            steps: [
              { label: 'Go to Company from the sidebar, then switch to the Organogram tab.' },
              { label: 'The chart starts from structures with no parent and branches downwards.' },
              { label: 'Click a node to expand or collapse everything beneath it.' },
              { label: 'Counts of branches, departments and units are summarised above the chart.' },
            ],
          },
          { type: 'tip', body: 'A node shows the head of that structure when one has been set on the structure record. To see employees arranged by who they report to, use the Staff Organogram under Employees instead.' },
        ],
      },
    ],
  },
    ...
    articles: [
        ...
        content: [
          { type: 'text', body: 'The Payroll module covers the complete employee pay cycle. It is split across two sidebar items — Salary (setup and configuration) and Payroll (running and approving pay runs).' },
          {
            ...
            rows: [
              ...
              ['Payslips', 'Rendered on demand as PDFs — nothing is generated or stored in advance. Available for Approved and Completed runs; open the run to download per-employee, or employees download their own under User Reports → My Payslips'],
            ],
          },
          {
            type: 'table',
            ...
            rows: [
              ['Draft',              'Created but not yet calculated — generate the payroll to fill it'],
              ['Processing',         'Figures calculated and still editable'],
              ['Pending Approval',   'Submitted for review; with several stages it stays here until the last approver'],
              ['Approved',           'Authorised, but nothing is posted and nobody is paid until you finalise'],
              ['Bank Pending',       'Posted to the core banking system, awaiting its confirmation — not yet paid'],
              ['Completed',          'The bank confirmed payment. The only status meaning staff have been paid'],
              ['GL Failed',          'The journal could not be posted; nothing was sent. Fix the cause and retry'],
              ['Rejected',           'Rejected by an approver or by the bank; regenerate to start again'],
            ],
          },
          {
            type: 'table',
          ...
          { type: 'warning', body: 'Every active employee must have a pay grade and a bank account on their record before payroll can be processed. A missing bank account does not stop the payslip, but it does stop the whole run from being finalised — the journal will not balance without it.' },
        ],
      },
      {
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'warning', body: 'Ensure all employee pay grades and bank accounts are up to date before running payroll. An employee with no bank account blocks finalisation of the entire run, not just their own payment.' },
          { type: 'tip', body: 'A run stays editable until its journal reaches the general ledger. You can edit amounts while it is Processing, Pending Approval, Approved or Rejected; it locks only once it becomes Bank Pending or Completed.' },
        ],
      },
      {
        id: 'approve-payroll',
        ...
        content: [
          {
            ...
            steps: [
              ...
              { label: 'Click "Approve". The run moves to Approved status. Approving does NOT pay anyone — it authorises the run so it can be finalised.' },
              { label: 'If more than one approval stage is configured, the run stays Pending Approval and moves to the next approver. It becomes Approved only after the final stage.' },
              { label: 'Click "Reject" (with a reason) to send the run back. A rejected run must be regenerated before it can be submitted again.' },
              { label: 'Finally, click "Finalize" on the Approved run to post the journal to the general ledger. This is the step that sends money.' },
            ],
          },
          { type: 'warning', body: 'Approve and Finalize are two separate steps. Approving only authorises the run; nothing is posted to the general ledger and no payment is made until you click Finalize.' },
          { type: 'tip', body: 'You need the "Approve Payroll" permission to see the Approve, Reject and Finalize actions.' },
        ],
      },
      {
        id: 'payroll-status-lifecycle',
        title: 'Payroll Run Statuses',
        summary: 'What each payroll status means, and what moves a run from one to the next.',
        icon: CheckCircle2,
        content: [
          { type: 'text', body: 'A payroll run moves through a fixed sequence of statuses. Knowing which one a run is in tells you what has happened and what to do next.' },
          {
            type: 'table',
            headers: ['Status', 'What it means', 'What moves it on'],
            rows: [
              ['Draft',            'The run has been created but not yet calculated.', 'Generate the payroll'],
              ['Processing',       'Figures are calculated and editable.', 'Submit for Approval'],
              ['Pending Approval', 'Waiting on an approver. With several stages configured it stays here until the last one approves.', 'Approve (or Reject)'],
              ['Approved',         'Authorised, but nothing has been posted and nobody has been paid yet.', 'Finalize'],
              ['Bank Pending',     'The journal has been posted to the core banking system, which is running its own approval. Staff are NOT paid yet.', 'The bank confirms or rejects'],
              ['Completed',        'The bank confirmed the payment. This is the only status that means staff have been paid.', 'Nothing — this is the end state'],
              ['GL Failed',        'The journal could not be posted (for example an invalid account). Nothing was sent.', 'Fix the cause, then Retry GL Posting'],
              ['Rejected',         'Rejected either by an internal approver or by the core banking system.', 'Regenerate the run'],
            ],
          },
          { type: 'warning', body: 'Bank Pending is not the same as paid. The run has reached the bank, but the bank runs a second approval outside this system. Until it confirms, treat the payment as in flight.' },
          {
            type: 'steps', heading: 'If a run is stuck at Bank Pending',
            steps: [
              { label: 'Check with the core banking team whether the payment was approved on their side.' },
              { label: 'If it was approved but this system was never notified, open the run and use "Mark as Paid" to move it to Completed.' },
              { label: '"Mark as Paid" requires the "Approve Payroll" permission and only works on a Bank Pending run.' },
            ],
          },
          { type: 'tip', body: 'When GL posting is switched off (record-only mode, in Settings), no bank is involved and finalising takes a run straight to Completed.' },
        ],
      },
      {
        id: 'payslips',
        title: 'Viewing and Downloading Payslips',
        summary: 'Access individual payslips for an Approved or Completed payroll run.',
        icon: FileText,
        content: [
          {
            type: 'steps', heading: 'Accessing payslips',
            steps: [
              { label: 'Go to Payroll and open an Approved or Completed payroll run.' },
      ...
      },
      {
        ...
        content: [
          { type: 'text', body: 'Each pay grade can be assigned a medical plan that defines the maximum amount claimable per year for staff and dependents.' },
          {
            ...
            rows: [
              ['Pay Grade', 'The grade this limit applies to'],
              ['Currency',  'Currency of the limit'],
              ['Amount',    'One combined annual limit, shared between the employee and all their dependents'],
            ],
          },
    ...
    articles: [
      {
        ...
        content: [
          ...
          { type: 'tip', body: 'An employee can exist without a user account (e.g. a legacy record), but a user account can NOT exist without an employee — every account must be linked to one employee record, and an employee can have only one account.' },
        ],
      },
      {
        id: 'create-user',
        ...
        content: [
          {
            ...
            steps: [
              ...
              { label: 'Search for and select the employee this account belongs to. This is required — an account cannot be created without one.' },
              { label: 'Enter a Username. This is what the person types to sign in.' },
              { label: 'Select a Role. At least one is required; the role defines what they can see and do.' },
              { label: 'Optionally add individual permissions on top of the role, for access the role alone does not grant.' },
              { label: 'Click Save. A password is generated automatically and emailed to the employee — you do not set one.' },
            ],
          },
          { type: 'warning', body: 'The generated password is sent to the employee\'s work email, falling back to their personal email. If the employee record has neither, the account is still created but nobody receives the password — add an email to the employee first, then use Reset Password.' },
          {
            type: 'table',
            headers: ['If saving fails', 'What it means'],
            rows: [
              ['Employee not found',                          'The selected employee record no longer exists'],
              ['A user account already exists for this employee', 'That employee is already linked to an account — edit it instead'],
              ['At least one role is required',               'No role was selected'],
              ['One or more role IDs are invalid',            'A selected role has since been deleted — reload and try again'],
            ],
          },
          { type: 'tip', body: 'If the account is created but roles could not be assigned, you will be told so explicitly. The account exists but cannot do anything until you open it and set the roles.' },
      ...
      },
      {
        ...
        summary: 'Update a user\'s username, role and permissions, reset their password, or deactivate the account.',
        icon: UserCheck,
        content: [
          {
            ...
            steps: [
              ...
              { label: 'Update their username, status, role or individual permissions.' },
              { label: 'Click Save to apply changes.' },
              { label: 'To reset a password, close the form and choose "Reset Password" from the same three-dot menu — it is a separate action with its own dialog, not a field on this form.' },
            ],
          },
          ...
          { type: 'warning', body: 'The "Delete" action in the menu does not delete the account. It deactivates it, exactly as Deactivate does, so no history is lost.' },
          { type: 'tip', body: 'Names and email addresses are not edited here — they live on the employee record and are changed under Employees.' },
        ],
      },
    ],
  },
    ...
    articles: [
        ...
        content: [
          { type: 'text', body: 'The Payroll Summary report gives a line-by-line view of every employee\'s earnings and deductions for a completed payroll run. It is the primary tool for verifying a run before funds are disbursed.' },
          {
            ...
            steps: [
              ...
              { label: 'Click "Generate" on the Payroll Summary card. Export and print options appear inside the report once it opens.' },
              { label: 'In the dialog, select the payroll run you want to report on.' },
              { label: 'The report downloads with one row per employee: Total Earnings, Total Deductions and Net Pay. Tax is included within Total Deductions — there is no per-component breakdown in this report.' },
            ],
          },
          { type: 'tip', body: 'Only Completed or Approved payroll runs appear in the selector. Everything else is excluded — including Bank Pending, so a run you have just finalised will not be listed until the bank confirms it.' },
        ],
      },
      {
        id: 'payslip-report',
      ...
      },
      {
        ...
        content: [
          { type: 'text', body: 'The Leave Utilisation report gives HR a full picture of how leave entitlement is being used across the organisation. It is useful for planning, compliance, and spotting patterns.' },
          {
            ...
            steps: [
              { label: 'Go to Admin Reports.' },
              { label: 'Click "Generate" on the Leave Utilization card. Export and print options appear inside the report once it opens.' },
              { label: 'The report shows one row per employee per leave type, covering application activity: requests made, approved days, pending days and rejections.' },
            ],
          },
          {
            type: 'table',
            ...
            rows: [
              ['Employee',        'Staff name'],
              ['ID',              'Employee number'],
              ['Leave Type',      'One row per employee per leave type'],
              ['Requests',        'How many applications were made'],
              ['Approved Days',   'Days from approved applications'],
              ['Pending Days',    'Days from applications still awaiting approval'],
              ['Rejected',        'How many applications were rejected'],
            ],
          },
        ],
      },
      {
        ...
        content: [
          { type: 'text', body: 'The Headcount report gives a snapshot of employee distribution by department. It is useful for workforce planning and management reporting.' },
          {
            ...
            steps: [
              { label: 'Go to Admin Reports.' },
              { label: 'Click "Generate" on the Department Headcount card. Export and print options appear inside the report once it opens.' },
              { label: 'The report lists each department with its Headcount, the number Active, and that department as a % of Workforce.' },
            ],
          },
      ...
      },
      {
        ...
        content: [
          ...
          },
          {
            ...
            rows: [
              ['Employee / ID',    'Staff name and employee number'],
              ['Pay Grade',        'Assigned salary grade, which determines the limit'],
              ['Currency',         'Currency of the limit and the amounts'],
              ['Medical Limit',    'The annual limit for that pay grade'],
              ['Staff Used',       'Approved claims for the employee themselves'],
              ['Dependent Used',   'Approved claims for their dependents'],
              ['Total Used',       'Staff Used + Dependent Used — both draw on the same limit'],
              ['Balance',          'Medical Limit − Total Used'],
              ['Utilisation %',    'How much of the limit has been consumed'],
            ],
          },
        ],
      },
      ...
      // ── User Reports ──
      {
        ...
        content: [
          ...
          { type: 'tip', body: 'Only payroll runs with status "Completed" or "Approved" appear in the list. A run that has just been finalised sits at Bank Pending until the bank confirms payment, and its payslip is not available during that time.' },
          { type: 'warning', body: 'If a payslip is missing, the most likely reason is that the run is still Bank Pending — awaiting confirmation from the bank. If other months are also missing, check with HR that your employee record is linked to your user account.' },
        ],
      },
      {
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'tip', body: 'Staff and dependent claims are listed together in one table, with a Type column showing which is which. Both draw on the same medical limit.' },
        ],
      },
    ],
  },
        ...
        content: [
          { type: 'text', body: 'Every company document has a sharing scope that determines which employees can see it in their Personal Info → Documents view. The scope is set when creating or editing a document.' },
          {
            ...
            rows: [
              ...
              ['None selected',                        'EVERY employee sees it. Leaving the sharing fields blank does not keep a document private — it publishes it to all staff. Select at least one department, employee or level to restrict it.'],
            ],
          },
        ...
        content: [
          { type: 'text', body: 'Employee Documents store personal records linked to a specific employee — passports, National IDs, driver\'s licences, SSNIT cards, and tax certificates. Each record tracks the issue date, expiry date, and an optional file attachment.' },
          {
            ...
            steps: [
              ...
              { label: 'Upload the document file. The limit is 20 MB per file.' },
    ...
    articles: [
      {
        ...
        content: [
          ...
          { type: 'warning', body: 'Whether nominations pass through a supervisor at all is a global setting (Settings → Controls → Approvals). The default is direct, meaning every nomination goes straight to Pending HR Approval and no supervisor stage occurs. Switch it to supervisor-first if you want that stage. Supervisor and Admin nominations always skip it regardless.' },
        ],
      },
      {
        id: 'training-catalog',
    ...
    articles: [
      ...
      },
      {
        ...
        content: [
          ...
          { type: 'warning', body: 'Clocking in is only possible inside the working hours set for your shift. Arrive before the window opens and you are told "Working hours have not started"; arrive after it closes and clock-in is shut until the next work day. This applies to both the web app and the kiosk. Ask an administrator to record the day manually if you are legitimately outside the window.' },
          { type: 'warning', body: 'Punches within 60 seconds of your previous punch are rejected to prevent accidental double-clicks. This guard applies to the web app; the kiosk does not have it.' },
          { type: 'tip', body: 'When "Require Photo" is switched on, clocking opens a camera instead of the usual confirmation dialog — the photo is the confirmation.' },
        ],
      },
      {
